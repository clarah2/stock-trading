...
class StockEnv:
    ...
    def reset():
        ...
        self.total_assets = sum([x*y for x,y in zip(self.close_price, self.stocks)])
        self.technical_indicators = [self.rsi, self.macd, self.obv, self.cci, self.adx]
        state = [self.balance] + self.stocks + self.open_price + self.close_price + self.technical_indicators
    ...
    def step():
        ...
        stocks_value = [x*y for x,y in zip(self.close_price, self.stocks)]
        daily_total_assets = sum(stocks_value) + self.balance
